...
/**
  ...
  * @ClassName:      SerialSettingsActivity
...
public class SerialSettingsActivity extends AppCompatActivity {
    ...
    private static final String TAG = "SerialSettingsActivity";
    ...
    private void initControl(){
        ...
        mSwitchStoreMode.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                try {
                    ((MApplication) getApplication()).serialAgent.setStorageMode(isChecked);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
        });
        mTvAppNameVersion.setText(Utils.getVerName(SerialSettingsActivity.this));
    }
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.btn_time_sync:
                    if (!NetUtils.isConnected(SerialSettingsActivity.this)) {//no network
                ...
                case R.id.btn_clear_database:
                    AlertDialog.Builder builder = new AlertDialog.Builder(SerialSettingsActivity.this);
                ...
                case R.id.btn_sleep_delay:
                    ...
                    try {
                        ((MApplication) getApplication()).serialAgent.setSleepDelay(delay);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                    break;
                ...
                case R.id.btn_dtc_show:
                    showDTC(SerialSettingsActivity.this,((MApplication) getApplication()).mAppData.getDtcList());
                    break;
                case R.id.btn_dtc_clear:
                    mBtnShowDTC.setEnabled(false);
                    try {
                        ((MApplication) getApplication()).serialAgent.requestClearDTC();
                    }catch (IOException e)
                    {
                        e.printStackTrace();
                    }
                ...
                case R.id.btn_device_ota:
                    startActivity(new Intent(getApplicationContext(), SerialOTAActivity.class));
                    break;
                ...
                case R.id.btn_ptr_mode:
                    if(mBtnPTRMode.getText().toString().equals(getString(R.string.btn_enter_ptr_mode))) {
                        AlertDialog.Builder builder2ptr = new AlertDialog.Builder(SerialSettingsActivity.this);
                        builder2ptr.setTitle(R.string.enter_ptr_mode_dialog_title)
                                ...
                                .setPositiveButton(R.string._ok, new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        ...
                                        try {
                                            ((MApplication) getApplication()).serialAgent.setWorkMode(CommandEnum.WorkMode.PTR_MODE);
                                        } catch (IOException e) {
                                            e.printStackTrace();
                                        }
                                        dialog.dismiss();
                        ...
                        AlertDialog.Builder builder2eld = new AlertDialog.Builder(SerialSettingsActivity.this);
                        builder2eld.setTitle(R.string.enter_eld_mode_dialog_title)
                                ...
                                .setPositiveButton(R.string._ok, new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        ...
                                        mBtnPTRMode.setEnabled(false);
                                        try {
                                            ((MApplication) getApplication()).serialAgent.setWorkMode(CommandEnum.WorkMode.ELD_MODE);
                                        } catch (IOException e) {
                                            e.printStackTrace();
                                        }
                                        dialog.dismiss();
                ...
                case R.id.btn_bus_protocol:
                    switch (busProtocolSelect)
                    {
                        case 0:
                            try {
                                ((MApplication) getApplication()).serialAgent.setBusProtocol(CommandEnum.BusProtocol.J1939_Protocol);
                            } catch (IOException e) {
                                e.printStackTrace();
                            }
                            break;
                        case 1:
                            try {
                                ((MApplication) getApplication()).serialAgent.setBusProtocol(CommandEnum.BusProtocol.J1587_Protocol);
                            } catch (IOException e) {
                                e.printStackTrace();
                            }
                            break;
                ...
                case R.id.btn_enter_adc_and_gpio:
                    Intent enter_adc_intent = new Intent(SerialSettingsActivity.this, SerialGpioActivity.class);
    ...
    public void requestSync(int beginMinutes, int endMinutes) {
        ...
        try {
            ((MApplication) getApplication()).serialAgent.requestDataSync(beginMinutes, endMinutes);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    */
    static class UiHandler extends Handler {
        WeakReference<SerialSettingsActivity> settingsActivityWeakReference;

        UiHandler(SerialSettingsActivity settingsActivity) {
            ...
            queryBaseInfoThread = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        Thread.sleep(100);
                        ((MApplication) getApplication()).serialAgent.requestSleepDelay();
                        Thread.sleep(100);
                        ((MApplication) getApplication()).serialAgent.requestWorkMode();
                        Thread.sleep(100);
                        ((MApplication) getApplication()).serialAgent.requestDeviceSN();
                        Thread.sleep(100);
                        ((MApplication) getApplication()).serialAgent.requestBusProtocol();
                        Thread.sleep(100);
                    } catch (IOException e) {
                        e.printStackTrace();
                    ...
                    while(true){
                        if(queryBaseInfoThread == null || queryBaseInfoThread.isInterrupted())
                            break;
                        try {
                            ((MApplication) getApplication()).serialAgent.requestDTC();
                            Thread.sleep(10000);
                        } catch (IOException e) {
                            e.printStackTrace();
                        } catch (InterruptedException e) {
    ...
    private void showAsJ1939(boolean show){
        if(show){
            ...
            //adc & gpio 暂时只有OBD有 adc & gpio are temporarily only available to OBD
            mTvAdcAndGpioTitle.setVisibility(View.VISIBLE);
            mBtnEnterAdcAndGpio.setVisibility(View.VISIBLE);
        }
    }
