 ...
 * @Version: 1.0
 */
public class BleGpioActivity extends AppCompatActivity {
    //Debug
    private static final String TAG = "BleGpioActivity";
    ...
    private void setGpioValue(int gpioValue,int gpioPos){
        if(gpioValue == 1)
            ((MApplication)getApplication()).getService().setGpioOutHigh(gpioSetCommandArray(gpioPos));
        else if(gpioValue == 0) {
            ((MApplication)getApplication()).getService().setGpioOutLow(gpioSetCommandArray(gpioPos));
        }
    }
                ...
                case sGpioDataIntent: {
                    ...
                    switch (responseType) {
                        ...
                        case Response.GetAdcValResponse:
                            ...
                            if(channel == 3) {
                                if(vref == 3.36f)
                                    value = ((MApplication) getApplication()).getService().computeAdcVal_for_channel3(data);
                                else if(vref > 0)
                                    value = ((MApplication) getApplication()).getService().computeAdcVal(data,vref,0.5f);
                                else
                                    ToastUtils.show(R.string.toast_reference_voltage_more_than_zero);
                            }
                            else {
                                if(vref == 3.36f)
                                    value = ((MApplication) getApplication()).getService().computeAdcVal(data);
                                else if(vref > 0)
                                    value = ((MApplication) getApplication()).getService().computeAdcVal(data,vref,0.0f);
                                else
                                    ToastUtils.show(R.string.toast_reference_voltage_more_than_zero);
                            }
