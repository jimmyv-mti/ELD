...
import com.android.eldbox_api.BleOTAHelper;

/**
  *
  * @ProjectName:
  * @Package:        com.android.eldbox
  * @ClassName:      BleOTAActivity
...
public class BleOTAActivity extends AppCompatActivity{
    ...
    private void init() {
        ...
        BluetoothDevice device = ((MApplication)getApplication()).getService().getDevice();
        if (device != null) {
            mTvDeviceName.setText(device.getName());
        } else {
            mTvDeviceName.setText(getString(R.string.ota_no_device));
        }
    }
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.btn_start_ota: {
                    if (BleOTAHelper.getInstance().initialize(((MApplication)getApplication()).getService(), otaFile, mEldboxOTACallBack)) {
